...
In the details panel, OpenCode sessions use exact token usage from OpenCode's session database (shown as `tokens:`). Non-OpenCode sessions fall back to PTY stream-based estimates (shown as `tokens~`).
