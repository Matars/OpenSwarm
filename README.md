...
- **Worktree graph** -- see all worktrees as an interactive graph with parent-child relationships, dirty/committed/local-only/pushed/merged-with-parent badges, ahead/behind counts, live agent activity, and estimated PTY token telemetry (ctx/out + tok/s)
